function obj = modelDownsampleRawMovies(obj)
	...
	try
		...
		downsampleSettings = inputdlg(...
			{...
				'Folder(s) where raw HDF5s are located [from previous prompts]. Use comma to separate multiple source folders:',...
...
function [newFolderListCell] = subfxnSelectFolders()
	...
	switch inputMethod
		...
		case 'manually enter folders to list'
			...
			% inputdlg
			newFolderList = inputdlgcol('One new line per folder path. Enter folder path WITHOUT any single/double quotation marks around the path.','Adding folders to calciumImagingAnalysis object for downsampling.',[21 150],{''},AddOpts);
			if isempty(newFolderList)
				warning('No folders given. Please re-run modelDownsampleRawMovies.')
				return
			end
		...
		case 'GUI select folders'
			...
			while ~isempty(pathToAdd)|length(newFolderListCell)<1
				try
					if ischar(pathToAdd)
						disp('Select a folder to add for downsampling. Press cancel to stop adding folders.')
						pathToAdd = uigetdir(pathToAdd,'Select a folder to add for downsampling. Press cancel to stop adding folders.');
