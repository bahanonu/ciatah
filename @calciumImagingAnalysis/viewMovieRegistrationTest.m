function obj = viewMovieRegistrationTest(obj)
	...
	try
		...
		normalizeMovieOpt = str2num(movieSettings{8});
		...
		for thisFileNumIdx = 1:nFilesToAnalyze
			...
			[inputMovieRegAll{thisFileNumIdx}{1}] = createMontageMovie(inputMovieRegAllTmp,'normalizeMovies',zeros([length(inputMovieRegAll) 1])+normalizeMovieOpt,'downsampleFactorSpace',montageDownsampleFactorSpace,'identifyingText',split(num2str(0:nTestToRun),'  '),'fontSize',15,'rotateMoviesText',1,'flipdimsText',1);
