...
- This user guide contains instructions to setup, run, and troubleshoot `CIAPKG`.
- Note: `CIAPKG` is a class (`calciumImagingAnalysis`) with various GUIs to allow processing of calcium imaging data. In addition, users can access the underlying `CIAPKG` functions to make custom workflows. See [Custom command-line pipelines](api_example_pipeline.md).
