function error = vennX( data, resolution, inputLabels )
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   elseif length( data ) == 7
       ...
       error = plot_venn3( data(1), data(2), data(3), data(4), data(5), data(6), data(7), ...
           resolution, dist_A_B, dist_B_C, dist_A_C, inputLabels );
       error = data - error';
   else
...
function error = plot_venn3( a, b, c, d, e, f, g, resolution, dist_A_B, dist_B_C, dist_A_C, inputLabels )
    ...
    img = img + 4.1 .* ( (X - center2_x).^2 + (Y - center2_y).^2 < r2^2 );
    ...
    text_x = mean(j);
    text_y = mean(i);
    h = text( text_x, text_y, [inputLabels{1} 10 num2str( a )] );
    ...
    text_x = mean(j);
    text_y = mean(i);
    h = text( text_x, text_y, [inputLabels{2} 10 num2str( c )] );
    set( h, 'FontWeight', 'bold','fontsize', fontSize )
    ...
    text_x = mean(j);
    text_y = mean(i);
    h = text( text_x, text_y, [inputLabels{3} 10 num2str( e )]);
    set( h, 'FontWeight', 'bold','fontsize', fontSize )
