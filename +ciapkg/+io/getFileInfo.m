function [fileInfo] = getFileInfo(fileStr, varargin)
	...
	% UPDATES:
    % 04-01-2021: Fixed the assay list to match the lab's experiments
    % 04-01-2021: Fixed the subject list to include D1- and D2- cre animals
    % 06-19-2021: Use folder name to determine the assay instead of the
    % folder path (solve "PA" problem in "USER_PATH")

	%========================
	
	options.assayList = {'EXT|Ext|RST|Rst|PA|SA|REN|REINST|CuedRST|CuedRst|Baseline|baseline'};
	%options.assayList = {...
	...
	options.originalStr = {'PAV-PROBE','SAL','SULP','SHC','REINST','EXT','PA', 'CuedRST', 'CuedRst','Rst', 'baseline'};
	options.replaceStr = {'PAVQ','SCH','SUL','SCH','RST','EXT','PA','RST','RST','RST', 'Baseline'};
