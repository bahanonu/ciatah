function [optsNC] = getNoRMCorreParams(movieDims,varargin)
	...
	% Changelog
		% 2022.09.19 [12:53:09] - Change default parameters and run final output through NoRMCorre parameter function to reduce chance of errors.
	% TODO
		%
	...
	try
		...
		if isempty(fieldnames(options.priorOpts))
			...
			optsNC2.grid_size =  [128,128]; %[128,128]
			optsNC2.mot_uf =  4;
	...
	try
		...
		% Run options through NoRMCorre param function to make sure final format is correct.
		optsNC = normcorre.NoRMCorreSetParms(...
			'd1', d1-bound,...
			'd2', d2-bound,...
			'grid_size',optsNC.grid_size,...
			'overlap_pre',optsNC.overlap_pre,...
			'min_patch_size',optsNC.min_patch_size,...
			'min_diff',optsNC.min_diff,...
			'us_fac',optsNC.us_fac,...
			'mot_uf',optsNC.mot_uf,...
			'max_dev',optsNC.max_dev,...
			'overlap_post',optsNC.overlap_post,...
			'max_shift',optsNC.max_shift,...
			'phase_flag',optsNC.phase_flag,...
			'shifts_method',optsNC.shifts_method,...
			'upd_template',optsNC.upd_template,...
			'init_batch',optsNC.init_batch,...
			'bin_width',optsNC.bin_width,...
			'buffer_width',optsNC.buffer_width,...
			'method',optsNC.method,...
			'iter',optsNC.iter,...
			'boundary',optsNC.boundary,...
			'add_value',optsNC.add_value,...
			'use_parallel',optsNC.use_parallel,...
			'memmap',optsNC.memmap,...
			'mem_filename',optsNC.mem_filename,...
			'mem_batch_size',optsNC.mem_batch_size,...
			'print_msg',optsNC.print_msg,...
			'plot_flag',optsNC.plot_flag,...
			'make_avi',optsNC.make_avi,...
			'name',optsNC.name,...
			'fr',optsNC.fr,...
			'output_type',optsNC.output_type,...
			'h5_groupname',optsNC.h5_groupname,...
			'h5_filename',optsNC.h5_filename,...
			'tiff_filename',optsNC.tiff_filename,...
			'output_filename',optsNC.output_filename,...
			'use_windowing',optsNC.use_windowing,...
			'window_length',optsNC.window_length,...
			'bitsize',optsNC.bitsize,...
			'correct_bidir',optsNC.correct_bidir,...
			'nFrames',optsNC.nFrames,...
			'bidir_us',optsNC.bidir_us,...
			'col_shift',optsNC.col_shift...
		);
	catch err
		disp(repmat('@',1,7))
		disp(getReport(err,'extended','hyperlinks','on'));
